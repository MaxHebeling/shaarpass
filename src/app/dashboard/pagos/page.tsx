...
export default async function PagosPage() {
  ...
  const enabled = Boolean(org?.payouts_enabled);   // totalmente habilitado (ya recibe depósitos)
      ...
      <div className="glass mt-8 rounded-3xl p-7">
        <div className="flex items-start gap-4">
          <span className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl ${enabled || sellingWhileVerifying ? "bg-emerald-500/15 text-emerald-300" : connected ? "bg-gold/15 text-gold" : "brand-gradient text-ink"}`}>
            {enabled || sellingWhileVerifying ? <CheckCircle2 className="h-6 w-6" /> : connected ? <AlertCircle className="h-6 w-6" /> : <Wallet className="h-6 w-6" />}
          </span>
          <div className="flex-1">
            <h2 className="font-display text-xl font-semibold">
              {enabled ? "Pagos activos" : sellingWhileVerifying ? "Ya puedes vender" : connected ? "Casi listo" : "Conecta Stripe"}
            </h2>
            <p className="mt-1 text-sm text-muted">
              {enabled
                ? "Tu cuenta está lista. El dinero de cada venta llega directo a tu cuenta y a tu banco."
                : sellingWhileVerifying
                ? "Tu cuenta ya puede cobrar: cada venta va directo a tu cuenta de Stripe. Stripe está terminando de verificar tus datos para liberar los depósitos a tu banco (suele tardar unas horas)."
                : connected
            ...
            <div className="mt-5">
              ...
              {(enabled || sellingWhileVerifying) && (
                <span className="inline-flex items-center gap-2 rounded-full bg-emerald-500/10 px-4 py-2 text-sm font-medium text-emerald-300">
                  <Zap className="h-4 w-4" /> {enabled ? "Listo para cobrar" : "Listo para vender"}
                </span>
              )}
            </div>
          </div>
